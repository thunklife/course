...
filter ::
  ...
  -> List a
  -> List a
filter f = foldRight go Nil
  where go x a | f x       = x:.a
               | otherwise = a
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = foldRight consOptional (Full Nil)

consOptional :: Optional a -> Optional (List a) -> Optional (List a) 
consOptional Empty _            = Empty
consOptional _ Empty            = Empty
consOptional (Full x) (Full xs) = Full (x :. xs)
